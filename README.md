...
融云Demo中使用redPacketlibrary模块集成了红包SDK相关红能。
redPacketlibrary介绍
...
initTokenData(RPValueCallback callback)
...
注意：App Server提供的获取签名的接口必须先验证用户身份，并保证签名的用户和该登录用户一致，防止该接口被滥用。详见云账户REST API开发文档
注册RedPacketModule
