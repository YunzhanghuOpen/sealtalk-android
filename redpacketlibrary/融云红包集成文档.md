...
**redpacketlibrary**，在融云**sdk2.7.0dev**的基础上提供了收发红包和零钱页的功能。
...
* 在Application的onCreate()方法中注册红包消息、回执消息类以及消息展示模板。
...
### 3.4 在扩展栏中增加红包按钮（SealAppContext）
...
```java
   ...
   mGroupId = groupID;
   //同步群信息
   AsyncTaskManager.getInstance(mContext).request(REQUEST_SYNCGROUP, SealAppContext.this);          int number = SharedPreferencesContext.getInstance().
     getSharedPreferences().getInt(groupID, 0);
